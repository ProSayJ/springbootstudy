 ...
 * 2.	使用java.io.File类的listF工les(FileFilter）和isDirectory方法，编一
 * 写个返回指定目录下所有子目录的方法。使用lambda表达式来代替FileFilter对象，再将它改写为一个方法引用。
 * 3.	使用java.工o.File类的list(FilenameFilter）方法，编写一个返回指定目录下、具有指定扩展名的所有文件。使用lambda表达式（而不是FilenameFilter) 来实现。它会捕获闭合作用域中的哪些变量？
