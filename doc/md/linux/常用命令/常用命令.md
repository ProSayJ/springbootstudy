...
---
    通常，利用SSH管理远程Linux服务器时，经常需要与本地交互文件。当然，我们可以利用FTP方式，比如通过Filezilla客户端软件。
    不过直接使用SSH软件（SecureCRT、Xshell）自带的上传和下载功能无疑使最方便快捷的。通常SSH软件支持的文件传输协议主要有ASCII、Xmodem、Zmodem等。
 ...
 **查看文件内容**
    ...
    less    somefile 可以翻页查看，下翻一页（空格）	上翻一页(b)上翻一行（向上箭头） 下翻一行（向下箭头） 可以搜索关键字（/keyword）
...
**文件权限的操作**
    ...
    linux文件权限的描述格式解读:
    drwxr-xr-x    d rwx r-x r-x   （也可以用二进制表示  111 101 101  -->  755）
    第一个字母：文件的类型常用的三种类型：
        D：表示目录;
        -：表示二进制文件;
...
**修改文件权限**
    ...
    也可以用数字的方式来修改权限:
     ...
     -R参数是递归 处理目录下的所有文件以及子文件夹
      ...
      注意: 有写保护的文件是可以删除的，删除有写保护的文件并不代表你修改了它。删除文件是修改了父目录的内容，而不是待删除文件的内容。
...
**查看文件或文件夹的大小**
    ...
    du -sh  /usr/src/redis-4.0.11.tar.gz
